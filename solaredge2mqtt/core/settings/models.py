...
class LocationSettings(BaseModel):
    ...
    def latitude_value(self) -> float:
        if self.latitude is None:
            raise ConfigurationException("location", "Latitude is not configured")
    ...
    def longitude_value(self) -> float:
        if self.longitude is None:
            raise ConfigurationException("location", "Longitude is not configured")
...
class ServiceSettings(BaseModel):
    ...
    def is_weather_enabled(self) -> bool:
        if self.weather.is_configured and not self.location.is_configured:
            logger.warning(
