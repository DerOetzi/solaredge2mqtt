...
class MonitoringSettings(BaseModel):
    ...
    def site_id_secret(self) -> str:
        if self.site_id is None:
            raise ConfigurationException("monitoring", "Site ID is not set correctly")
    ...
    def username_value(self) -> str:
        if self.username is None:
            raise ConfigurationException(
                "monitoring", "Monitoring username is not set correctly"
            )
    ...
    def is_configured(self) -> bool:
        return all(
            [
                self.site_id is not None and bool(self.site_id_secret),
                self.username is not None,
                self.password is not None and bool(self.password_secret),
            ]
        )
