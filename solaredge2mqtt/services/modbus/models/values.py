...
class ModbusComponentValueGroup(Solaredge2MQTTBaseModel, MixinModbusSunSpecScaleValue):
    @classmethod
        ...
        for phase in ["l1", "l2", "l3"]:
            ...
            if current_key in payload:
                values[phase] = cls.scale_value(
                    payload, current_key, "current_scale")
    ...
    def extract_sunspec_payload(cls, payload: SunSpecPayload) -> dict[str, float]:
        ...
        for phase in ["l1", "l2", "l3"]:
            ...
            if voltage_key in payload:
                values[phase] = cls.scale_value(
                    payload, voltage_key, "voltage_scale")
...
class ModbusACPower(ModbusComponentValueGroup):
    ...
    def parse_sunspec_payload_power_key(
        ...
        power_key: Literal["power_ac", "power"]
    ) -> dict[str, float]:
        return {
