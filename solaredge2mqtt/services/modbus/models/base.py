...
class ModbusDeviceInfo(Solaredge2MQTTBaseModel):
    ...
    @classmethod
    def from_sunspec(
        cls, payload: SunSpecPayload, unit: ModbusUnitInfo | None = None
    ) -> ModbusDeviceInfo:
...
class ModbusComponent(Component, MixinModbusSunSpecScaleValue, ABC):
    ...
    def from_sunspec(cls, info: ModbusDeviceInfo, payload: SunSpecPayload) -> Self:
        ...
        return cls.model_validate({"info": info, **values})
    ...
    def mqtt_topic(self, has_followers: bool = False) -> str:
        unit_key = self.info.unit.key if has_followers and self.info.unit else None
