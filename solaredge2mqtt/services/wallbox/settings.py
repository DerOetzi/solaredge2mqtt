...
class WallboxSettings(BaseModel):
    ...
    def password_secret(self) -> str:
        if self.password is None:
            raise ConfigurationException(
                "wallbox", "Wallbox password not set correctly"
            )
    ...
    def serial_secret(self) -> str:
        if self.serial is None:
            raise ConfigurationException("wallbox", "Wallbox serial not set correctly")
    ...
    def is_configured(self) -> bool:
        return all(
            [
                ...
                self.password is not None and self.password_secret,
                self.serial is not None and self.serial_secret,
            ]
        )
