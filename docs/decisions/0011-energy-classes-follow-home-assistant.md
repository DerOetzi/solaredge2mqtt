...
**Consequence:** The long-term statistic for this entity changes from mean/min/max to sum. Home Assistant raises a repair issue for the existing entity once the new discovery config is picked up, and the old measurement-type statistics have to be deleted under Developer Tools → Statistics before the sum statistic starts. This is a one-time manual step for existing installations and is documented under [Home Assistant](../configuration/home-assistant.md#after-upgrading-the-ev-charger-session-energy-statistic).
