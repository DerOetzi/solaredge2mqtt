...
class TestHTTPClientAsync:
    ...
    @pytest.mark.asyncio
    async def test_get_raises_when_session_stays_none(self):
        """Test _get raises when init does not initialize a session."""
        client = HTTPClientAsync("TestService")
    ...
    async def test_get_with_401_calls_login(self):
        ...
        with patch.object(client, "init"):
            client.session = mock_session
            result = await client._get("https://test.com", login=login_callback)
    ...
    async def test_post_connection_error_returns_none(self):
        ...
        with patch.object(client, "init"):
            client.session = mock_session
            result = await client._post("https://test.com", json={"key": "value"})

        assert result is None

    @pytest.mark.asyncio
    async def test_post_raises_when_session_stays_none(self):
        """Test _post raises when init does not initialize a session."""
        client = HTTPClientAsync("TestService")

        with patch.object(client, "init", return_value=None):
            with pytest.raises(RuntimeError, match="HTTP session not initialized"):
                await client._post("https://test.com", json={"key": "value"})
    ...
    @pytest.mark.asyncio
    async def test_get_cookie_iterates_until_match(self):
        """Test get_cookie continues scanning until a later cookie matches."""
        client = HTTPClientAsync("TestService")
        client.init()
